...
#' two columns of allele dplyr::counts (farmed and wild) taken only from the "train"
...
compute_epp <- function(D) {
  D %>%
    ...
    dplyr::group_by(chrom, coord, variant, group, allele) %>%
    dplyr::summarise(counts = n()) %>%
    tidyr::spread(data = ., key = group, value = counts, fill = 0) %>%
    dplyr::rename(farmed_cnts = farmed,
           wild_cnts = wild) %>%
    dplyr::group_by(chrom, coord, variant) %>%
    dplyr::mutate(farmed_freq = (farmed_cnts + 1/2) / sum(farmed_cnts + 1/2),
