...
they looked correct.
7. After all that, I decided that it was easiest to keep everything in a tidy format, so this also creates
compressed output files: `tidy-ns.rds` and `tidy-west.rds` in `intermediates/01`.
...
So, how about a function to read plink ped and map files to tidy format.  We have written
one of those in its own file, so that it will be easier to package it up later.
```{r plink-to-tidy}
source("R/plink2tidy.R")
```
Here is a listing of it:
```r
`r paste(readLines("R/plink2tidy.R"), collapse = "\n")`
```
