...
a different population.  But it looks little funky.  There are some individuals looking pure in there that should not be.
...
```
Turn out that this is a problem during more than just initialization---you end up with salmon that are clearly F1s being called
pure.

Let's try using fewer markers
```{r}
create_hybrid_dataset(SAR, wild_pop = "LPR", hyb_cat = "F1", L = 1500, dir = "hyb_dir")
```
