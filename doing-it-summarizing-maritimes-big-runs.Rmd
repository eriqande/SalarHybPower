---
title: "Summarizing the Runs --- Maritimes"
output: 
  html_notebook:
    toc: true
    toc_float: true
...
Let's do this with the Maritimes as that is all I have the simulations for yet.
...
mar_res <- read_rds("outputs/MAR_long_runs_tibble.rds") %>%
...
Our starting point for this will be the `mar_res` tibble, which are all the results
...
### Accuracy
```{r}
squashed_accu_df <- mar_squashed_cats %>%
...
```{r}
squashed_effi_df <- mar_squashed_cats %>%
  group_by(pop, true_hyb_cat, num_loci) %>%
  dplyr::do(effi_func(pp = .$post_prob, 
                      ihc = .$inferred_hyb_v_non_hyb,
                      thc = .$actual_hyb_v_non_hyb))
...
dump <- lapply(unique(rocdf$pop), function(x) {
  ...
  ggsave(g, filename = paste0("outputs/roc_MAR_", x, ".pdf"))
})
```

I made PDFs of all of those, but here is how it looks for SMA:
```{r sma_png1, fig.width=11, fig.height=11}
ggplot(rocdf %>% filter(pop == "SMA"), aes(x = false_pos_rate, y = true_pos_rate, colour = num_loci)) +
    geom_line() +
    facet_grid(true_hyb_cat ~ focal_hyb_cat) + 
    ggtitle("BDN") +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5))
```
I saved these in the files `roc_MAR_XXX.pdf`.  Note that "infd" means "inferred" in the
...
dump <- lapply(unique(rocdf$pop), function(x) {
  ...
  ggsave(g, filename = paste0("outputs/roc_MAR_pcolor", x, ".pdf"))
})
```

These are saved in the files, "roc_MAR_pcolorXXX.pdf". Here is what it looks like
for SMA:
```{r sma_png2, fig.width=11, fig.height=11}
 ggplot(rocdf %>% filter(pop == "SMA"), aes(x = false_pos_rate, y = true_pos_rate, colour = pthresh,
                                              group = num_loci)) +
    geom_line() +
    facet_grid(true_hyb_cat ~ focal_hyb_cat) + 
    ggtitle("BDN") +
